    ...
    def generate_runtime_dockerfile(self, graph, stack_config=None):
        ...
        for pkg, meta in graph.items():
            ...
            if pkg == "icu":
                ...
                df += "    export CXXFLAGS=\"$CXXFLAGS -g0 -O1 -fstack-protector-strong -D_FORTIFY_SOURCE=2\" && \\\n"
                ...
                df += "    mkdir -p /artifacts/usr/share/doc/$LIB_NAME && \\\n"
                df += "    find . -maxdepth 2 -type f \\( -iname \"license*\" -o -iname \"copying*\" -o -iname \"mit-license*\" \\) -exec cp -v {} /artifacts/usr/share/doc/$LIB_NAME/ \\; -quit; \\\n"
        ...
        if stack_type == "binary_injection":
            ...
            df += "      if [ \"$RUNTIME_NAME\" = \"php\" ]; then dnf install -y php-cli; fi && \\\n"
            ...
            df += "      else \\\n"
            df += "        SRC_DIR=$(find /tmp/extract -mindepth 1 -maxdepth 1 -type d | head -n 1) && \\\n"
            df += "        if [ -n \"$SRC_DIR\" ]; then \\\n"
            ...
            df += f"    mkdir -p /runtime-root/usr/share/doc/{runtime.get('name')} && \\\n"
            df += f"    find . -maxdepth 2 -type f \\( -iname \"license*\" -o -iname \"copying*\" -o -iname \"mit-license*\" \\) -exec cp -v {{}} /runtime-root/usr/share/doc/{runtime.get('name')}/ \\; -quit && \\\n"
