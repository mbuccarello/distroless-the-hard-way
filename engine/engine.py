    ...
    def generate_runtime_dockerfile(self, graph, stack_config=None):
        ...
        for pkg, meta in graph.items():
            ...
            if pkg == "icu":
                ...
                df += "    mkdir -p /opt/distroless && echo '--- DEBUG: Contents of /opt/distroless ---' && ls -R /opt/distroless && \\\n"
        ...
        if stack_type == "binary_injection":
            ...
            df += "      cp -rv /usr/lib64/libphp* /runtime-root/usr/lib64/ && \\\n"
            df += "      cp -rv /etc/php* /runtime-root/etc/ && \\\n"
