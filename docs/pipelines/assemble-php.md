...
# Pipeline Strategy: Opensource Distroless PHP Image

The `assemble-php` pipeline creates a high-performance, distroless execution environment for PHP applications (specifically PHP-FPM), layered on top of the Opensource Distroless `cc` foundation.
...
2.  **Stage 2: Runtime Acquisition**: 
    -   Fetches a verified PHP source tarball (Architecture: x86_64).
...
3.  **Stage 3: Distroless Assembly**: 
    ...
    -   Integrates the PHP-FPM binary and core libraries into `/usr/local/php`.
    -   Ensures zero-trust compliance by including only the necessary bits for high-performance PHP execution.

## Zero-Trust Proof Points

### 1. Proof of Mathematical Ancestry
The pipeline proof-of-origin for its base layer (`cc:latest`) is established before assembly. This mathematically links the PHP image to the verified foundational layers (`glibc`, `openssl`, `zlib`, `tzdata`).
...
| Artifact | Purpose |
| :--- | :--- |
| **OCI Image** | `ghcr.io/${{ github.repository_owner }}/php:latest` |
| **SCA Scan** | **Trivy** scan and SBOM generation. |
| **Signing** | The PHP image is signed via **Cosign**. |
