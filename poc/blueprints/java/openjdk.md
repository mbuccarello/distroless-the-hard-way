...
While Opensource-Distroless demands that **System OS Libraries** (like `glibc` and `openssl`) be iteratively compiled from pure C-code to permanently sever the dependency on external OS maintainers (Debian/Alpine), **Application Libraries** represent a different threat footprint.
