...
The `base` image is the absolute root of the Distroless-The-Hard-Way architecture. In a traditional container, this layer would be `FROM debian` or `FROM alpine`. In Distroless-The-Hard-Way, we achieve absolute sovereignty by replacing the host OS with a strictly compiled tier of foundational libraries.
