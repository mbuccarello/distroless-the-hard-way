...
COPY --from=ncurses /artifacts/usr /cc-root/usr
COPY --from=readline /artifacts/usr /cc-root/usr
COPY --from=libxml2 /artifacts/usr /cc-root/usr
COPY --from=sqlite /artifacts/usr /cc-root/usr
COPY --from=oniguruma /artifacts/usr /cc-root/usr
COPY --from=krb5 /artifacts/usr /cc-root/usr
COPY --from=curl /artifacts/usr /cc-root/usr
COPY --from=libxcrypt /artifacts/usr /cc-root/usr
COPY --from=bzip2 /artifacts/usr /cc-root/usr
COPY --from=pcre2 /artifacts/usr /cc-root/usr
COPY --from=libffi /artifacts/usr /cc-root/usr
