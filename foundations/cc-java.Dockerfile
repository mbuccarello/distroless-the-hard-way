...
COPY --from=bzip2 /artifacts/usr /cc-root/usr
COPY --from=libpng /artifacts/usr /cc-root/usr
COPY --from=freetype2 /artifacts/usr /cc-root/usr
COPY --from=libjpeg-turbo /artifacts/usr /cc-root/usr
COPY --from=lcms2 /artifacts/usr /cc-root/usr
COPY --from=libx11 /artifacts/usr /cc-root/usr
COPY --from=libxext /artifacts/usr /cc-root/usr
COPY --from=libxrender /artifacts/usr /cc-root/usr
COPY --from=libxtst /artifacts/usr /cc-root/usr
COPY --from=alsa-lib /artifacts/usr /cc-root/usr
