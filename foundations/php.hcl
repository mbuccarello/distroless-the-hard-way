...
target "curl" {
  ...
  args = {
    ...
    LIB_CONFIG = "--with-openssl=/opt/distroless --with-zlib=/opt/distroless --with-nghttp2=/opt/distroless --with-ca-bundle=/etc/ssl/certs/ca-certificates.crt --without-libpsl"
  }
  contexts = {
    builder = "target:foundations"
    brotli = "target:brotli"
