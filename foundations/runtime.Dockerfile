...
COPY --from=libffi /artifacts/usr /opt/distroless
COPY --from=expat /artifacts/usr /opt/distroless
COPY --from=bzip2 /artifacts/usr /opt/distroless
COPY --from=xz /artifacts/usr /opt/distroless
COPY --from=ncurses /artifacts/usr /opt/distroless
COPY --from=readline /artifacts/usr /opt/distroless
COPY --from=sqlite /artifacts/usr /opt/distroless
ARG RUNTIME_NAME=python
ARG RUNTIME_URL
RUN set -ex && mkdir -p /tmp/extract && \
    if [ "$RUNTIME_URL" = "DNF" ]; then \
      dnf clean all && dnf install -y --setopt=install_weak_deps=False $RUNTIME_NAME $RUNTIME_NAME-fpm $RUNTIME_NAME-mysqlnd $RUNTIME_NAME-opcache $RUNTIME_NAME-xml $RUNTIME_NAME-mbstring $RUNTIME_NAME-gd $RUNTIME_NAME-curl && \
      mkdir -p /runtime-root/usr/bin /runtime-root/usr/sbin /runtime-root/usr/lib64 /runtime-root/etc && \
      cp -rv /usr/bin/php* /runtime-root/usr/bin/ && \
      cp -rv /usr/sbin/php* /runtime-root/usr/sbin/ && \
      cp -rv /usr/lib64/libphp* /runtime-root/usr/lib64/ && \
      cp -rv /etc/php* /runtime-root/etc/ && \
      echo "Runtime installed via dnf"; \
    else \
      curl -L "$RUNTIME_URL" -o /tmp/runtime.tar.gz && \
      tar -xf /tmp/runtime.tar.gz -C /tmp/extract && \
