...
RUN set -ex && mkdir -p /tmp/extract && \
    if [ "$RUNTIME_URL" = "DNF" ]; then \
      ...
      find /runtime-root/usr/bin/ /runtime-root/usr/sbin/ /runtime-root/usr/lib64/ /runtime-root/usr/lib/ -type f 2>/dev/null | while read -r file; do \
