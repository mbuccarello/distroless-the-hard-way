...
ENV CACHE_BYPASS_SETUP="1789547306.772414"
RUN set -ex && curl -L "https://www.php.net/distributions/php-8.3.13.tar.xz" -o source.tar.gz && mkdir src && tar -xf source.tar.gz -C src --strip-components=1 && cd src && \
    ...
    mkdir -p /runtime-root/usr/share/doc/php && \
    find . -maxdepth 2 -type f \( -iname "license*" -o -iname "copying*" -o -iname "mit-license*" \) -exec cp -v {} /runtime-root/usr/share/doc/php/ \; -quit && \
...
ENV CACHE_BYPASS="1789547306.7725189"
